...
case class Field[VP, MP, CSP <: CState](constraints: Constraints[VP, MP, CSP]) {
  ...
  def req(error: Error = null) = Field(constraints.req(err(error, Error("required"))))
  ...
  def parse(map: Map[String, Seq[String]], validate: Boolean, name: Name): FieldState[VP, MP, CSP] = {
    ...
    constraints.handler.parse(view) match {
      case Left(e) => FieldStateWithoutModel[VP, MP, CSP](name, view, constraints)(validate).addErrors(e)
      case Right(m) => FieldStateWithModel[VP, MP, CSP](name, view, constraints, m)(validate)
    }
  }

  def fill(model: MP, validate: Boolean, name: Name): FieldState[VP, MP, CSP] = {
    val view = constraints.handler.format(model)
    FieldStateWithModel[VP, MP, CSP](name, view, constraints, model)(validate)
  }
